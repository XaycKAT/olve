...
G4VPhysicalVolume* DetectorConstruction::DefineVolumes()
{
  ...
  int nofLayers=1; //количество слоев ячеек
  ...
  G4double silicOut=plasOut*nofCell;
  G4double silicIn=silicOut-silicThick;
  G4double silicLength=nofCell*wolfLength;	//длина призмы 
  G4double silicLengthF=2*mm;	//выступающий слой
  G4double fullSilicLength=silicLength+silicLengthF;
  G4double fullSilicLengthF=silicLength+2*silicLengthF;
  ...
  G4double zPlaneSilic[numZPlanesSilic] = { 0,silicLengthF,silicLengthF,fullSilicLength,fullSilicLength,fullSilicLengthF};
  /*int boxX=0.1*cm;
  int boxY=1*cm;
  int boxZ=1*cm;
  G4double boxStep=1*cm;
  G4double boxRange=0.6*cm;*/
  // Get materials
  ...
  G4Material* siliconMat = G4Material::GetMaterial("G4_SILICON_DIOXIDE");
...
/*
  new G4PVReplica(
                 ...
                 nofCell,        // number of replica
                  2*M_PI/6.);  // witdth of replica
*/
  ...
  {	
	for(  int k = -sideL+1; k < sideL; k++ )
	{
		...
		//possitionCells[check].push_back(Tm);
  ...
  G4LogicalVolume* silicLV
    = new G4LogicalVolume(
                 Silic,        // its solid
                 siliconMat, // its materialM_PI/6.*rad
                 "silicLV");          // its name
  ...
  G4VPhysicalVolume* silicPV
    = new G4PVPlacement(
                 zRot,                // no rotation
                 G4ThreeVector((nofCell/2.-1/2.)*2*plasOut,0, 0), // its position
...
/*  G4Box* cellBox = new  G4Box("cellBox", boxX, boxY, boxZ);
                         
  G4LogicalVolume* cellBoxLV
    = new G4LogicalVolume(
                 cellBox,           // its cell
                 defaultMaterial,  // its material
                 "CellBox");         // its name	
  ...
  // Define one layer as one assembly volume
	G4AssemblyVolume* boxDetector = new G4AssemblyVolume();
	boxDetector->AddPlacedVolume( cellBoxLV, Tr1 );
	for(int i=0;i<4;i++)
	{
		G4ThreeVector Tm1(-plasOut-boxZ/2.-i*boxRange,0,0);
		G4RotationMatrix* zRot = new G4RotationMatrix; // Rotates X and Z axes only
 		zRot->rotateZ(M_PI/6.*rad); 
		G4RotationMatrix invRot = zRot->invert();
		Tr1 = G4Transform3D(Rm,Tm1);
		boxDetector->MakeImprint( worldLV, Tr1 );
	}
  
  //silicon box
  
  G4Box* aBox = new G4Box("aBox", boxX, boxY, boxZ);

  G4LogicalVolume* aBoxLV
    = new G4LogicalVolume(
                 aBox,        // its solid
  ...
  G4VPhysicalVolume* aBoxPV
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0, 0., 0), // its position
                 aBoxLV,       // its logical volume                         
                 "aBoxPV",           // its name
                 cellBoxLV,          // its mother  volume
                 false,            // no boolean operation
                 0,                // copy number
                 fCheckOverlaps);  // checking overlaps */
